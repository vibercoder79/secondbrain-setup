> # Vault-Hub fuer Codex — Spiegel von CLAUDE.md mit Codex-Brille
> Diese Datei gehoert nach `~/Obsidian/SecondBrain/AGENTS.md`.
> Inhalt ist identisch zu CLAUDE.md, nur "Claude" durch "Codex" ersetzt.
> Codex CLI laedt diese Datei automatisch.
>
...
- `03 Bereiche/`: Laufende Verantwortungsbereiche ohne Enddatum. Jeder Bereich ist ein eigener Ordner, weil Bereiche ueber die Zeit wachsen und mehrere Dateien sammeln. Enthaelt u.a. `Skills/` — die zentrale Dokumentation aller Codex Skills (qualitative Doku, Zusatzinfos, Versionshistorie). Die technische Implementation liegt in `~/.codex/skills/`, der Code auf GitHub.
...
- `05 Daily Notes/`: Taegliches Logbuch. Was an einem Tag passiert ist, welche Entscheidungen getroffen wurden, was offen ist. Gibt Codex die Kontinuitaet zwischen Sessions.
...
- WICHTIG — Dieses Vault wird von mehreren KIs genutzt (Codex, ChatGPT, Claude, Gemini, Grok, Perplexity, ...). Damit klar ist, welche KI welchen Eintrag erstellt hat und der Nutzer jederzeit zum Original-Chat zurueckspringen kann, gilt:
- JEDER Eintrag MUSS `source: codex` und `chat_url: <link-oder-"unbekannt">` im Frontmatter enthalten. Wenn du die Chat-URL nicht kennst, schreibe `chat_url: unbekannt` — niemals weglassen
- Bei Daily Notes: Markiere deinen Abschnitt mit `> [!info] Via Codex — [Original-Chat](url)`
- Andere KIs haben ihre eigene Config-Datei (CLAUDE.md, GEMINI.md, ...) mit den gleichen Tracking-Regeln
...
- Wenn der Nutzer sagt "merk dir das" oder "speicher das", speichere es dort wo es thematisch hingehoert. Schreibregeln nach `00 Kontext/Schreibstil.md`, Projekt-Infos in die jeweilige Projekt-Datei, technische Erkenntnisse in `04 Ressourcen/`, Vault-Regeln in diese AGENTS.md. Im Zweifel kurz fragen wo es hin soll.
...
3. **Tags im Frontmatter** — pro beruehrtem Projekt ein Tag (plus Standard `daily`). Beispiel:
   ...
   tags:
     ...
     - mein-projekt
   ```
4. **Wikilink zum Projekt-Hub** — in jeder Projekt-Sektion oben: `> [!info] Via Codex — [[<Projekt> PMO HUB]]`
...
> [!info] Via Codex — [[Beispiel-Projekt - PMO HUB]]
...
> [!info] Via Codex — [[Mein Projekt PMO HUB]]
...
`[[00 Kontext/Workflows/Projekt-Anlegen]]` — Single Source of Truth fuer alle KIs (Codex, Claude, Gemini, ...). Diese Datei ersetzt die internen References von KI-Skills als Wissens-Quelle.
...
- Tracking-Regeln (`source: codex`, `chat_url: ...`) gelten in jeder einzelnen Datei
...
Die qualitative Dokumentation aller Codex Skills liegt zentral in `03 Bereiche/Skills/`. Das SecondBrain ist die Single Source of Truth fuer Skill-Wissen — nicht der Skill-Ordner selbst.

### Drei-Ebenen-Modell

| Ebene | Zweck | Ort |
|-------|-------|-----|
| Implementation | SKILL.md, scripts/, templates/ | `~/.codex/skills/` |
...
- Frontmatter: `tags: [codex-skill]`, `skill`, `version`, `aktualisiert`, `status`
